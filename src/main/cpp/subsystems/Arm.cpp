...
void Arm::ArmInit() {
	...
	m_RunFlywheel.OnTrue(new frc2::InstantCommand([&] {
		...
		m_ShooterMotor1.SetControl(m_VelocityDutyCycle.WithVelocity(-1 * m_FlywheelVelocity * 1.05 * 2));
		m_ShooterMotor2.SetControl(m_VelocityDutyCycle.WithVelocity(m_FlywheelVelocity * m_Differential * 1.05 * 2));
	})).OnFalse(new frc2::InstantCommand([&] {
